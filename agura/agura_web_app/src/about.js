            ...
            <div className='flex text-white'>
                ...
                {/* This is the div of the form.*/}
                    <div className='flex justify-center my-8'>
                        <form action="" className='space-y-2 w-full mx-8 rounded'>
                            
                        <div className='flex items-center'><label htmlFor="names">Full names<span className='text-red-500'>*</span></label></div>
                            <input type="text" placeholder='Ex: Nzaramba Innocent' id='names' required className='block w-full p-3 bg-transparent border focus:outline-none focus:border-sky-500 focus:ring-1 rounded-full'/>
                        
                            <div className='block w-full'><label htmlFor="message">Email<span className='text-red-500'>*</span></label></div>
                            <input type="text" placeholder='Ex: sample@gmail.com' id='email' required className='block w-full p-3 bg-transparent border focus:outline-none focus:ring-1 focus:border-sky-500 rounded-full'/>
                            <div className='block w-full'><label htmlFor="message">Message<span className='text-red-500'>*</span></label></div>
                            <textarea name="message" id="message" cols="30" rows="10" placeholder='Message' className='block w-full p-3 text-black focus:ring-1 focus:outline-none focus:border-sky-500 rounded'></textarea>
                            <div className='mt-6'>
                                <button type='submit' className='border  rounded-full px-10 py-4 bg-gray-600 text-color_1 hover:border-slate-600 hover:bg-transparent hover:text-gray-600 hover:translate-y-2 transform transition'>Submit</button>
                            </div>
                        </form>
                    </div>
                </div>
